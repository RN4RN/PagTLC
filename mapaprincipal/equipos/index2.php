...
if (!$stmt->execute()) {
    echo "Error al registrar historial: " . $stmt->error;
    return false;
}
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $stmt->bind_param("sssiissssssii", $nombre_equipo, $descripcion, $tipo_equipo, $cantidad_total, 
                      $cantidad_disponible, $serie, $estado, $estacion, $marca, $modelo, $tip_equip, $usuario['id_usuario'], $id_equipo);
} else {
    // Insertar nuevo equipo
    $sql = "INSERT INTO equipos (nombre_equipo, descripcion, tipo_equipo, cantidad_total, cantidad_disponible, serie, estado, estacion, marca, modelo, tip_equip, creado_por)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
    
    $stmt = $conexion->prepare($sql);
    $stmt->bind_param("sssiissssssi", $nombre_equipo, $descripcion, $tipo_equipo, $cantidad_total, 
                      $cantidad_disponible, $serie, $estado, $estacion, $marca, $modelo, $tip_equip, $usuario['id_usuario']);
}
        ...
        if ($stmt->execute()) {
            ...
            registrarCambio($conexion, 'creacion', $detalle);
            
            echo json_encode(['success' => true, 'message' => 'Equipo agregado correctamente!', 'id_nuevo' => $id_nuevo]);
        } else {
...
if (isset($_GET['eliminar'])) {
    ...
    // Primero obtenemos los datos del equipo para el historial
...
<div class="totalmente">
    ...
    <form id="formEquipo">
        ...
        <input type="text" name="tip_equip" id="tip_equip" placeholder="Tipo equipo (abreviado)" required>
          ...
          <div class="botones">
            <button type="submit" class="btn-guardar" id="btnGuardar">Guardar Editado</button>
    ...
    <?php if ($result->num_rows > 0): ?>
        ...
        data-creador="<?= htmlspecialchars($row['creador_nombre'] ?: 'Desconocido') ?>"
        data-editor="<?= htmlspecialchars($row['editor_nombre'] ?: 'No editado') ?>"
        data-fecha-creacion="<?= $row['fecha_creacion_formatted'] ?>"
        data-fecha-edicion="<?= $row['fecha_edicion_formatted'] ?>"
...
<div id="ventanaDerecha" style="background-color:rgba(39, 39, 39, 0.94); filter: blur(4);">
  ...
  <h3 style="color:#17a2b8; display: inline; ">Estacion: </h3>
  <p id="EstacionEquip" style="color: white; display: inline;">Estacion del equipo</p><br>
  <h3 style="color:#17a2b8; display: inline; ">Serie: </h3>
  <p id="SerieEquip" style="color: white; display: inline;">Serie del equipo</p><br>
  <h3 style="color:#17a2b8; display: inline; ">Marca: </h3>
  <p id="MarcaEquip" style="color: white; display: inline;">Marca del equipo</p><br>
  <h3 style="color:#17a2b8; display: inline; ">Modelo: </h3>
  <p id="modelos" style="color: white; display: inline;">modelo del equipo</p> <br>
  <h3 style="color:#17a2b8; display: inline; ">Tipo de equipo: </h3>
  <p id="TipoEquip" style="color: white; display: inline;">Tipo del equipo</p> <br>
  <h3 style="color:#17a2b8; display: inline; ">Tipo: </h3>
  <p id="tipo" style="color: white; display: inline;">tipo de equipo</p> <br>
  <h3 style="color:#17a2b8; display: inline; ">Descripcion: </h3>
  <p id="descripciones" style="color: white; display: inline;">descripcion del equipo</p> <br>
  <h3 style="color:#17a2b8; display: inline; ">Cantidad disponible: </h3>
<p id="disponible" style="color: white; display: inline;">15</p> <br>
<h3 style="color:#17a2b8; display: inline; ">Cantidad total: </h3>
<p id="total" style="color: white; display: inline;">20</p> <br>
<h3 style="color:#17a2b8; display: inline; ">Estado:</h3>
<p id="EstadoEquip" style="color: white; display: inline;">Estado del equipo</p><br>

<div class="barra-estado-contenedor" style="background-color: #ccc; width: 300px; height: 10px; display: inline-block; border-radius: 10px; overflow: hidden; margin-top: 5px;">
  <div id="barraEstadoVentana" class="barra-estado" style="background-color: gray; height: 100%; width: 100%;"></div>
</div>
<h3 style="color:#b2eccf;" >Modificaciones de este equipo</h3>
<h3 style="color:#17a2b8; display: inline; ">Creado por: </h3>
<p id="creadorNombre" style="color: white; display: inline;"></p><br>

<h3 style="color:#17a2b8; display: inline; ">Última edición por: </h3>
<p id="editorNombre" style="color: white; display: inline;"></p><br>
...
const creador = boton.getAttribute('data-creador');
    ...
    // Muestra la información del creador y editor
    ...
    switch(estado) {
        ...
        case 'Aceptable':
            ...
            barraEstado.style.width = '60%';
            break;
        case 'Malo':
            barraEstado.style.backgroundColor = '#dc3545';
            barraEstado.style.width = '30%';
            break;
        default:
