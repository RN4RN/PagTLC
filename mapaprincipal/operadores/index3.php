...
// CORRECCIÓN: Usar el campo 'perfil' que ya tienes para la imagen del sidebar.
...
// Se usará directamente el campo 'perfil' para las imágenes.
...
// --- OBTENER TODOS LOS MOVIMIENTOS CON TIPO DE EQUIPO PARA JS ---
$allMovements = [];
// CORRECCIÓN: JOIN con la tabla 'equipos' para obtener 'tipo_equipo'
$sqlAllMovements = "SELECT m.id_movimiento, m.id_equipo, m.id_usuario, m.cantidad, m.fecha_salida, 
                           m.fecha_entrega, m.estado_entrega, m.observacion, m.nombre_equipo, m.nombre_usuario,
                           e.tipo_equipo 
                    FROM movimientos m
...
<head>
    ...
    <link rel="stylesheet" href="estyles3.css"> <!-- Asegúrate que esta ruta es correcta -->
    <link rel="icon" href="favicon.ico" type="image/x-icon"> <!-- Asegúrate que esta ruta es correcta -->
    ...
    <style>
        ...
        /* Estilo para imagen de perfil en tarjeta y detalles */
        .user-image, #detail-user-image {
            width: 100%; /* o un tamaño fijo como 80px */
            height: auto; /* o un tamaño fijo como 80px */
            object-fit: cover;
            border-radius: 50%; /* Si quieres imágenes redondas */
        }
        /* En el sidebar */
        .profile-section img {
        ...
        <div class="sidebar-content">
            ...
            <div class="profile-section">
                <button class="profile-button" id="profile-button"
                    ...
                    <!-- CORRECCIÓN: Usar $sidebarUserData['perfil'] para la imagen -->
                    <img src="<?= htmlspecialchars($sidebarUserData['perfil'] ?: '../uploads/perfiles/default_avatar.png') ?>"
                         alt="Avatar de <?= htmlspecialchars($sidebarUserData['nombre']) ?>"
                         onerror="this.onerror=null; this.src='../uploads/perfiles/default_avatar.png';">
                    <span>Mi Perfil</span>
                </button>
                ...
                <div class="cards-grid">
                    <?php while ($row = $resultAllUsers->fetch_assoc()): ?>
                        <?php
                            ...
                            $userDni = $row['dni'] ?? 'N/A';
                            // CORRECCIÓN: Usar $row['perfil'] para la imagen de la tarjeta. Asumir una ruta base si es necesario.
                            $userPerfilSrc = !empty($row['perfil']) ? htmlspecialchars($row['perfil']) : '../uploads/perfiles/default_avatar.png';
        ...
        <section class="right-panel" aria-labelledby="user-details-heading">
            ...
            <div id="user-details-content" class="user-details" style="display: none;">
                <div class="user-header">
                    <img id="detail-user-image" src="../uploads/perfiles/default_avatar.png" alt="Foto del usuario"
                         onerror="this.onerror=null; this.src='../uploads/perfiles/default_avatar.png';">
                    <div>
                ...
                <div class="user-activity-section">
                    ...
                    <div class="activity-buttons">
                        <button id="btn-view-movements" class="button"><i class="fas fa-exchange-alt"></i> Ver Movimientos y Gráfico</button>
                    </div>
...
<script>
// Asumiendo que tienes una URL base para tus imágenes de perfil si no son rutas completas
const BASE_PROFILE_IMAGE_URL = '../uploads/perfiles/'; // Ajusta esta ruta si es necesario
...
const usersData = {
    ...
    if ($resultAllUsers->num_rows > 0) {
        ...
        while ($row = $resultAllUsers->fetch_assoc()) {
            if (!$first) echo ',';
            $first = false;
            // CORRECCIÓN: El campo 'perfil' ya contiene la ruta/nombre del archivo
            $profilePath = !empty($row['perfil']) ? $row['perfil'] : 'default_avatar.png';
            // Si 'perfil' es solo el nombre del archivo, y no una ruta completa o relativa desde la raíz web:
            // $profileUrl = BASE_PROFILE_IMAGE_URL . $profilePath;
            // Si 'perfil' ya es una ruta utilizable (ej. 'uploads/perfiles/imagen.png' o una URL completa), úsala directamente.
            // Para este ejemplo, asumiré que 'perfil' es el nombre del archivo y necesita la BASE_URL.
            // Si tu campo 'perfil' ya tiene una ruta como 'uploads/perfiles/imagen.png', entonces no necesitas BASE_PROFILE_IMAGE_URL aquí.
            // Simplemente usa $profilePath directamente como $profileUrl.
            // Por tu descripción "lo guarda como con una ruta", usaré $profilePath directamente.
            ...
            $userData = [
                ...
                'perfil_url' => $profileUrl, // Usar la URL procesada
...
function displayUserMovements(userId) {
    ...
    if (userMovements && userMovements.length > 0) {
        userMovements.forEach(mov => {
            ...
            const fechaSalida = mov.fecha_salida ? new Date(mov.fecha_salida).toLocaleDateString('es-ES', { day: '2-digit', month: '2-digit', year: 'numeric' }) : 'N/A';
            let tipoMovimiento = mov.estado_entrega || 'Salida';
            if (mov.fecha_entrega && tipoMovimiento !== 'Devolución') { // Asumiendo que si hay fecha_entrega es una devolución o entrega completada
                 // Podrías necesitar una columna 'tipo_movimiento' en la tabla 'movimientos' para ser más preciso (Entrega/Devolución)
                 // Aquí, si hay fecha_entrega, y no es explícitamente "Devolución" por otro campo, podría ser ambiguo.
                 // Por ahora, si hay fecha_entrega, lo marcamos como "Completado" o usamos estado_entrega.
                 tipoMovimiento = `Completado (${mov.estado_entrega || 'N/A'})`;
            }


            listItem.innerHTML = `
                <span class="mov-equipo">${mov.nombre_equipo || 'Equipo Desconocido'} (Cant: ${parseInt(mov.cantidad, 10)})</span><br>
                <span class="mov-details">Fecha Salida: ${fechaSalida} - Tipo Equipo: ${mov.tipo_equipo || 'N/A'}</span><br>
                <span class="mov-details">Estado/Tipo Mov: ${tipoMovimiento}</span><br>
...
function renderUserActivityChart(userMovements, userName) {
    ...
    // CORRECCIÓN: Agrupar por tipo_equipo ('Consumible', 'Permanente')
    const equipmentTypeCounts = userMovements.reduce((acc, mov) => {
        const type = mov.tipo_equipo || 'Desconocido'; // 'Consumible' o 'Permanente'
        acc[type] = (acc[type] || 0) + parseInt(mov.cantidad, 10);
        return acc;
    }, {});

    const labels = Object.keys(equipmentTypeCounts); // Serán ['Consumible', 'Permanente', 'Desconocido']
    const dataValues = Object.values(equipmentTypeCounts).map(val => parseInt(val, 10)); // Asegurar enteros

    const data = {
        labels: labels,
        datasets: [{
            label: `Tipos de Equipos Movidos por ${userName}`,
            data: dataValues,
            backgroundColor: [
                'rgba(255, 159, 64, 0.7)', // Color para Consumibles
                'rgba(54, 162, 235, 0.7)',  // Color para Permanentes
                'rgba(150, 150, 150, 0.7)'  // Color para Desconocido
            ],
            borderColor: [
                'rgba(255, 159, 64, 1)',
                'rgba(54, 162, 235, 1)',
    ...
    activityChartInstance = new Chart(ctx, {
        ...
        options: {
            responsive: true,
            scales: {
                y: { // Eje Y para las cantidades
                    beginAtZero: true,
                    title: { display: true, text: 'Cantidad Total Movida' },
                    ticks: {
                        stepSize: 1, // Para asegurar que solo se muestren enteros
                        callback: function(value) { if (Number.isInteger(value)) { return value; } }
                    }
                },
                x: { // Eje X para los tipos de equipo
                     title: { display: true, text: 'Tipo de Equipo' }
                }
            },
            plugins: {
                legend: { display: false },
                title: { display: true, text: `Resumen de Movimientos por Tipo de Equipo para ${userName}` }
            }
        }
    });
}
...
function mostrarDetallesUsuario(userId) {
    ...
    // CORRECCIÓN: Usar user.perfil_url
    userImg.src = user.perfil_url || '../uploads/perfiles/default_avatar.png';
    userImg.alt = `Foto de ${user.nombre}`;
    // El onerror ya está en el tag img, pero se puede reforzar aquí si es necesario
    userImg.onerror = function() { this.onerror=null; this.src='../uploads/perfiles/default_avatar.png'; };
    ...
    document.getElementById('btn-view-movements').onclick = () => {
        const userMovements = displayUserMovements(user.id_usuario);
        renderUserActivityChart(userMovements, user.nombre);
    };
...
// CORRECCIÓN: onerror para las imágenes. Asumir una ruta base para default_avatar si es necesario.
const DEFAULT_AVATAR_PATH = '../uploads/perfiles/default_avatar.png'; // Ajusta si es necesario

document.addEventListener('error', (e) => {
    if (e.target.tagName === 'IMG' && 
        (e.target.classList.contains('user-image') || 
         e.target.id === 'detail-user-image' || 
         (e.target.parentNode && e.target.parentNode.classList.contains('profile-section'))
        )
       ) {
        // Verificar si la imagen que falló ya es la de por defecto para evitar bucles
        if (e.target.src !== DEFAULT_AVATAR_PATH) {
            e.target.src = DEFAULT_AVATAR_PATH; 
        }
        e.target.onerror = null; // Prevenir bucles si la imagen por defecto también falla
    }
}, true);
</script>
